...
class CardleKnownFactsRow extends StatelessWidget {
  final List<CarGuessResult> guesses;

  const CardleKnownFactsRow({super.key, required this.guesses});

  int _rank(CarMatch match) {
    switch (match) {
  ...
  Widget build(BuildContext context) {
    final columnCount = carColumnLabels().length;
    return Padding(
  ...
  Widget _summaryCell(int columnIndex) {
    ...
    final color = switch (best.match) {
      CarMatch.correct => kCarCorrectColor,
      CarMatch.partial => kCarPartialColor,
      CarMatch.incorrect => kCarIncorrectColor,
    };
    final display = best.match == CarMatch.correct ? best.value : '?';
