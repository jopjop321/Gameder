...
import '../models/blackclover_load_status.dart';
import '../services/blackclover_character_service.dart';
import '../theme/cloverdle_theme.dart';
import '../widgets/blackclover_dex_dialog.dart';
...
class CloverdleScreen extends StatefulWidget {
  ...
  @override
  State<CloverdleScreen> createState() => _CloverdleScreenState();
}

class _CloverdleScreenState extends State<CloverdleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  BlackcloverLoadStatus _status = BlackcloverLoadStatus.loading;
  List<BlackcloverCharacter> _availableCharacters = [];
  BlackcloverCharacter? _answer;
  final List<BlackcloverGuessResult> _guesses = [];
  List<BlackcloverCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = BlackcloverLoadStatus.loading);

    final characters = await BlackcloverCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = BlackcloverLoadStatus.error);
      return;
    }
    ...
    setState(() {
      ...
      _hasWon = false;
      _status = BlackcloverLoadStatus.ready;
    });
  ...
  void _submitGuess(BlackcloverCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != BlackcloverLoadStatus.ready || _isAnswerRevealed || _hasWon || alreadyGuessed) {
      return;
    }

    final result = BlackcloverGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kBlackcloverBgColor,
      appBar: AppBar(
        title: const Text('Cloverdle'),
        backgroundColor: Colors.transparent,
        foregroundColor: Colors.white,
        elevation: 0,
        actions: [
          if (_status == BlackcloverLoadStatus.ready)
            IconButton(
      ...
      body: SafeArea(
        child: _status == BlackcloverLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kBlackcloverAccentColor),
              )
            : _status == BlackcloverLoadStatus.error
                ? BlackcloverErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'blackclover_unlimitedGuesses'.tr,
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'blackclover_inputHint'.tr,
