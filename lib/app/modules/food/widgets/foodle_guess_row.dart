...
class FoodleGuessRow extends StatefulWidget {
  final FoodGuessResult result;
  final bool animate;

  const FoodleGuessRow({
    super.key,
  ...
  @override
  State<FoodleGuessRow> createState() => _FoodleGuessRowState();
}

class _FoodleGuessRowState extends State<FoodleGuessRow>
    ...
    return Padding(
      ...
      child: Row(
        children: List.generate(cells.length, (index) {
          return _FoodleFlipCell(
            animation: _flips[index],
            child: _FoodleCellContent(
  ...
  Color _matchColor(FoodMatch match) {
    switch (match) {
      case FoodMatch.correct:
        return kFoodCorrectColor;
      case FoodMatch.partial:
        return kFoodPartialColor;
      case FoodMatch.incorrect:
        return kFoodIncorrectColor;
    }
  }
}

class _FoodleFlipCell extends StatelessWidget {
  final Animation<double> animation;
  final Widget child;

  const _FoodleFlipCell({required this.animation, required this.child});
      ...
      builder: (context, _) {
        ...
        return Transform(
          ...
          child: isFrontVisible
              ? const _FoodleCellContent(value: '', color: kFoodHeaderCellColor)
              : Transform(
                  alignment: Alignment.center,
...
class _FoodleCellContent extends StatelessWidget {
  final String value;
  final Color color;

  const _FoodleCellContent({required this.value, required this.color});
