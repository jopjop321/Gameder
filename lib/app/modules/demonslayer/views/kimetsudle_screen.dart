...
import '../models/demonslayer_load_status.dart';
import '../services/demonslayer_character_service.dart';
import '../theme/kimetsudle_theme.dart';
import '../widgets/demonslayer_dex_dialog.dart';
...
class KimetsudleScreen extends StatefulWidget {
  ...
  @override
  State<KimetsudleScreen> createState() => _KimetsudleScreenState();
}

class _KimetsudleScreenState extends State<KimetsudleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  DemonslayerLoadStatus _status = DemonslayerLoadStatus.loading;
  List<DemonslayerCharacter> _availableCharacters = [];
  DemonslayerCharacter? _answer;
  final List<DemonslayerGuessResult> _guesses = [];
  List<DemonslayerCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = DemonslayerLoadStatus.loading);

    final characters = await DemonslayerCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = DemonslayerLoadStatus.error);
      return;
    }
    ...
    setState(() {
      ...
      _hasWon = false;
      _status = DemonslayerLoadStatus.ready;
    });
  ...
  void _updateSuggestions() {
    ...
    final guessedNames =
        _guesses.map((result) => result.character.name).toSet();
    setState(() {
  ...
  void _submitGuess(DemonslayerCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != DemonslayerLoadStatus.ready ||
        _isAnswerRevealed ||
        _hasWon ||
        alreadyGuessed) {
      return;
    ...
    final result = DemonslayerGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kDemonslayerBgColor,
      appBar: AppBar(
        title: const Text('Kimetsudle'),
      ...
      body: SafeArea(
        child: _status == DemonslayerLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kDemonslayerAccentColor),
              )
            : _status == DemonslayerLoadStatus.error
                ? DemonslayerErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'demonslayer_unlimitedGuesses'.tr,
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'demonslayer_inputHint'.tr,
