...
class KimetsudleGuessRow extends StatefulWidget {
  final DemonslayerGuessResult result;
  final bool animate;

  const KimetsudleGuessRow({
    super.key,
  ...
  @override
  State<KimetsudleGuessRow> createState() => _KimetsudleGuessRowState();
}

class _KimetsudleGuessRowState extends State<KimetsudleGuessRow>
  ...
  Color _matchColor(DemonslayerMatch match) {
    switch (match) {
      case DemonslayerMatch.correct:
        return kDemonslayerCorrectColor;
      case DemonslayerMatch.partial:
        return kDemonslayerPartialColor;
      case DemonslayerMatch.incorrect:
        return kDemonslayerIncorrectColor;
    }
  }
}

class _KimetsudleFlipCell extends StatelessWidget {
  final Animation<double> animation;
  final Widget child;

  const _KimetsudleFlipCell({required this.animation, required this.child});
...
class _KimetsudleNameCellContent extends StatelessWidget {
  final DemonslayerCharacter character;
  final Color color;

  const _KimetsudleNameCellContent(
      {required this.character, required this.color});
...
class _KimetsudleCellContent extends StatelessWidget {
  final String value;
  final Color color;

  const _KimetsudleCellContent({required this.value, required this.color});
