...
import '../models/reborn_load_status.dart';
import '../services/reborn_character_service.dart';
import '../theme/reborndle_theme.dart';
import '../widgets/reborn_dex_dialog.dart';
import '../widgets/reborn_error_state.dart';
import '../widgets/reborndle_guess_row.dart';
import '../widgets/reborndle_header_row.dart';
import '../widgets/reborndle_known_facts_row.dart';

class ReborndleScreen extends StatefulWidget {
  const ReborndleScreen({super.key});

  @override
  State<ReborndleScreen> createState() => _ReborndleScreenState();
}

class _ReborndleScreenState extends State<ReborndleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  RebornLoadStatus _status = RebornLoadStatus.loading;
  List<RebornCharacter> _availableCharacters = [];
  RebornCharacter? _answer;
  final List<RebornGuessResult> _guesses = [];
  List<RebornCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = RebornLoadStatus.loading);

    final characters = await RebornCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = RebornLoadStatus.error);
      return;
    }
    ...
    setState(() {
      ...
      _hasWon = false;
      _status = RebornLoadStatus.ready;
    });
  ...
  void _submitGuess(RebornCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != RebornLoadStatus.ready || _isAnswerRevealed || _hasWon || alreadyGuessed) {
      return;
    }

    final result = RebornGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Future<void> _confirmSurrender() async {
    if (_status != RebornLoadStatus.ready || _isAnswerRevealed || _hasWon) return;
    final shouldSurrender = await showSurrenderConfirmation(
      context,
      cardColor: kRebornHeaderCellColor,
    );
    if (!mounted || !shouldSurrender) return;
    _revealAnswer();
  }

  Future<void> _openRebornDex() async {
    final selected = await showRebornDexDialog(context, _availableCharacters);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kRebornBgColor,
      appBar: AppBar(
        title: const Text('Reborndle'),
        backgroundColor: Colors.transparent,
        foregroundColor: Colors.white,
        elevation: 0,
        actions: [
          if (_status == RebornLoadStatus.ready)
            IconButton(
              onPressed: _openRebornDex,
              icon: const Icon(Icons.menu_book_rounded),
              tooltip: 'reborn_dexTooltip'.tr,
            ),
          if (_status == RebornLoadStatus.ready && !_isAnswerRevealed && !_hasWon)
            IconButton(
      ...
      body: SafeArea(
        child: _status == RebornLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kRebornAccentColor),
              )
            : _status == RebornLoadStatus.error
                ? RebornErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'reborn_unlimitedGuesses'.tr,
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const ReborndleHeaderRow(),
            ReborndleKnownFactsRow(guesses: _guesses),
            ..._guesses.asMap().entries.map(
                  (entry) => ReborndleGuessRow(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        '${character.displayFamily} · ${character.displayRole}',
