...
import '../models/mha_guess_result.dart';
import '../models/mha_load_status.dart';
import '../services/mha_character_service.dart';
import '../theme/herodle_theme.dart';
import '../widgets/mha_dex_dialog.dart';
import '../widgets/mha_error_state.dart';
import '../widgets/herodle_guess_row.dart';
import '../widgets/herodle_header_row.dart';
import '../widgets/herodle_known_facts_row.dart';

class HerodleScreen extends StatefulWidget {
  const HerodleScreen({super.key});

  @override
  State<HerodleScreen> createState() => _HerodleScreenState();
}

class _HerodleScreenState extends State<HerodleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  MhaLoadStatus _status = MhaLoadStatus.loading;
  List<MhaCharacter> _availableCharacters = [];
  MhaCharacter? _answer;
  final List<MhaGuessResult> _guesses = [];
  List<MhaCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = MhaLoadStatus.loading);

    final characters = await MhaCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = MhaLoadStatus.error);
      return;
    }
  ...
  void _submitGuess(MhaCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != MhaLoadStatus.ready || _isAnswerRevealed || _hasWon || alreadyGuessed) {
      return;
    }

    final result = MhaGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Future<void> _confirmSurrender() async {
    if (_status != MhaLoadStatus.ready || _isAnswerRevealed || _hasWon) return;
    final shouldSurrender = await showSurrenderConfirmation(
      context,
      cardColor: kMhaHeaderCellColor,
    );
    if (!mounted || !shouldSurrender) return;
    _revealAnswer();
  }

  Future<void> _openMhaDex() async {
    final selected = await showMhaDexDialog(context, _availableCharacters);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kMhaBgColor,
      appBar: AppBar(
        title: const Text('Herodle'),
        backgroundColor: Colors.transparent,
        foregroundColor: Colors.white,
        elevation: 0,
        actions: [
          if (_status == MhaLoadStatus.ready)
            IconButton(
              onPressed: _openMhaDex,
              icon: const Icon(Icons.menu_book_rounded),
              tooltip: 'mha_dexTooltip'.tr,
            ),
          if (_status == MhaLoadStatus.ready && !_isAnswerRevealed && !_hasWon)
            IconButton(
      ...
      body: SafeArea(
        child: _status == MhaLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kMhaAccentColor),
              )
            : _status == MhaLoadStatus.error
                ? MhaErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'mha_unlimitedGuesses'.tr,
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const HerodleHeaderRow(),
            HerodleKnownFactsRow(guesses: _guesses),
            ..._guesses.asMap().entries.map(
                  (entry) => HerodleGuessRow(
