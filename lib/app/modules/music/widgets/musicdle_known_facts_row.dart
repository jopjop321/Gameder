...
class MusicdleKnownFactsRow extends StatelessWidget {
  final List<SongGuessResult> guesses;

  const MusicdleKnownFactsRow({super.key, required this.guesses});

  int _rank(SongMatch match) {
    switch (match) {
  ...
  Widget build(BuildContext context) {
    final columnCount = musicColumnLabels().length;
    return Padding(
  ...
  Widget _summaryCell(int columnIndex) {
    ...
    final color = switch (best.match) {
      SongMatch.correct => kMusicCorrectColor,
      SongMatch.partial => kMusicPartialColor,
      SongMatch.incorrect => kMusicIncorrectColor,
    };
    final display = best.match == SongMatch.correct ? best.value : '?';
