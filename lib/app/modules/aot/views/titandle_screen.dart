...
import '../models/aot_load_status.dart';
import '../services/aot_character_service.dart';
import '../theme/titandle_theme.dart';
import '../widgets/aot_dex_dialog.dart';
import '../widgets/aot_error_state.dart';
import '../widgets/titandle_guess_row.dart';
import '../widgets/titandle_header_row.dart';
import '../widgets/titandle_known_facts_row.dart';

class TitandleScreen extends StatefulWidget {
  const TitandleScreen({super.key});

  @override
  State<TitandleScreen> createState() => _TitandleScreenState();
}

class _TitandleScreenState extends State<TitandleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  AotLoadStatus _status = AotLoadStatus.loading;
  List<AotCharacter> _availableCharacters = [];
  AotCharacter? _answer;
  final List<AotGuessResult> _guesses = [];
  List<AotCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = AotLoadStatus.loading);

    final characters = await AotCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = AotLoadStatus.error);
      return;
    }
  ...
  void _submitGuess(AotCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != AotLoadStatus.ready || _isAnswerRevealed || _hasWon || alreadyGuessed) {
      return;
    }

    final result = AotGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Future<void> _confirmSurrender() async {
    if (_status != AotLoadStatus.ready || _isAnswerRevealed || _hasWon) return;
    final shouldSurrender = await showSurrenderConfirmation(
      context,
      cardColor: kAotHeaderCellColor,
    );
    if (!mounted || !shouldSurrender) return;
    _revealAnswer();
  }

  Future<void> _openAotDex() async {
    final selected = await showAotDexDialog(context, _availableCharacters);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kAotBgColor,
      appBar: AppBar(
        title: const Text('Titandle'),
        backgroundColor: Colors.transparent,
        foregroundColor: Colors.white,
        elevation: 0,
        actions: [
          if (_status == AotLoadStatus.ready)
            IconButton(
              onPressed: _openAotDex,
              icon: const Icon(Icons.menu_book_rounded),
              tooltip: 'aot_dexTooltip'.tr,
            ),
          if (_status == AotLoadStatus.ready && !_isAnswerRevealed && !_hasWon)
            IconButton(
      ...
      body: SafeArea(
        child: _status == AotLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kAotAccentColor),
              )
            : _status == AotLoadStatus.error
                ? AotErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'aot_unlimitedGuesses'.tr,
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const TitandleHeaderRow(),
            TitandleKnownFactsRow(guesses: _guesses),
            ..._guesses.asMap().entries.map(
                  (entry) => TitandleGuessRow(
