...
class TitandleKnownFactsRow extends StatelessWidget {
  final List<AotGuessResult> guesses;

  const TitandleKnownFactsRow({super.key, required this.guesses});

  int _rank(AotMatch match) {
    switch (match) {
  ...
  Widget build(BuildContext context) {
    final columnCount = aotColumnLabels().length;
    return Padding(
  ...
  Widget _summaryCell(int columnIndex) {
    ...
    final color = switch (best.match) {
      AotMatch.correct => kAotCorrectColor,
      AotMatch.partial => kAotPartialColor,
      AotMatch.incorrect => kAotIncorrectColor,
    };
    final display = best.match == AotMatch.correct ? best.value : '?';
