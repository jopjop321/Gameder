...
class OnepiecedleKnownFactsRow extends StatelessWidget {
  final List<OnePieceGuessResult> guesses;

  const OnepiecedleKnownFactsRow({super.key, required this.guesses});

  int _rank(OnePieceMatch match) {
    switch (match) {
  ...
  Widget build(BuildContext context) {
    final columnCount = onePieceColumnLabels().length;
    return Padding(
  ...
  Widget _summaryCell(int columnIndex) {
    ...
    final color = switch (best.match) {
      OnePieceMatch.correct => kOnePieceCorrectColor,
      OnePieceMatch.partial => kOnePiecePartialColor,
      OnePieceMatch.incorrect => kOnePieceIncorrectColor,
    };
    final display = best.match == OnePieceMatch.correct ? best.value : '?';
