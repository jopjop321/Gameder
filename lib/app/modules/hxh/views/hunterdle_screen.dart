...
import '../models/hxh_load_status.dart';
import '../services/hxh_character_service.dart';
import '../theme/hunterdle_theme.dart';
import '../widgets/hxh_dex_dialog.dart';
import '../widgets/hxh_error_state.dart';
import '../widgets/hunterdle_guess_row.dart';
import '../widgets/hunterdle_header_row.dart';
import '../widgets/hunterdle_known_facts_row.dart';

class HunterdleScreen extends StatefulWidget {
  const HunterdleScreen({super.key});

  @override
  State<HunterdleScreen> createState() => _HunterdleScreenState();
}

class _HunterdleScreenState extends State<HunterdleScreen> {
  final _random = Random();
  final _searchController = TextEditingController();
  final _searchFocusNode = FocusNode();
  final _tableScrollController = ScrollController();
  HxhLoadStatus _status = HxhLoadStatus.loading;
  List<HxhCharacter> _availableCharacters = [];
  HxhCharacter? _answer;
  final List<HxhGuessResult> _guesses = [];
  List<HxhCharacter> _suggestions = [];
  ...
  Future<void> _loadAndStartGame() async {
    setState(() => _status = HxhLoadStatus.loading);

    final characters = await HxhCharacterService.loadCharacters();

    if (!mounted) return;
    if (characters.isEmpty) {
      setState(() => _status = HxhLoadStatus.error);
      return;
    }
  ...
  void _submitGuess(HxhCharacter character) {
    final alreadyGuessed =
        _guesses.any((result) => result.character.name == character.name);
    if (_status != HxhLoadStatus.ready || _isAnswerRevealed || _hasWon || alreadyGuessed) {
      return;
    }

    final result = HxhGuessResult.evaluate(character, _answer!);
    setState(() {
  ...
  Future<void> _confirmSurrender() async {
    if (_status != HxhLoadStatus.ready || _isAnswerRevealed || _hasWon) return;
    final shouldSurrender = await showSurrenderConfirmation(
      context,
      cardColor: kHxhHeaderCellColor,
    );
    if (!mounted || !shouldSurrender) return;
    _revealAnswer();
  }

  Future<void> _openHxhDex() async {
    final selected = await showHxhDexDialog(context, _availableCharacters);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kHxhBgColor,
      appBar: AppBar(
        title: const Text('Hunterdle'),
        backgroundColor: Colors.transparent,
        foregroundColor: Colors.white,
        elevation: 0,
        actions: [
          if (_status == HxhLoadStatus.ready)
            IconButton(
              onPressed: _openHxhDex,
              icon: const Icon(Icons.menu_book_rounded),
              tooltip: 'hxh_dexTooltip'.tr,
            ),
          if (_status == HxhLoadStatus.ready && !_isAnswerRevealed && !_hasWon)
            IconButton(
      ...
      body: SafeArea(
        child: _status == HxhLoadStatus.loading
            ? const Center(
                child: CircularProgressIndicator(color: kHxhAccentColor),
              )
            : _status == HxhLoadStatus.error
                ? HxhErrorState(onRetry: _loadAndStartGame)
                : Column(
                    children: [
                      Padding(
                        padding: const EdgeInsets.fromLTRB(16, 0, 16, 8),
                        child: Text(
                          'hxh_unlimitedGuesses'.tr,
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const HunterdleHeaderRow(),
            HunterdleKnownFactsRow(guesses: _guesses),
            ..._guesses.asMap().entries.map(
                  (entry) => HunterdleGuessRow(
