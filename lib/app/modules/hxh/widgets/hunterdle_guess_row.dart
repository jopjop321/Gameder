...
class HunterdleGuessRow extends StatefulWidget {
  final HxhGuessResult result;
  final bool animate;

  const HunterdleGuessRow({
    super.key,
  ...
  @override
  State<HunterdleGuessRow> createState() => _HunterdleGuessRowState();
}

class _HunterdleGuessRowState extends State<HunterdleGuessRow>
  ...
  Color _matchColor(HxhMatch match) {
    switch (match) {
      case HxhMatch.correct:
        return kHxhCorrectColor;
      case HxhMatch.partial:
        return kHxhPartialColor;
      case HxhMatch.incorrect:
        return kHxhIncorrectColor;
    }
  }
}

class _HunterdleFlipCell extends StatelessWidget {
  final Animation<double> animation;
  final Widget child;

  const _HunterdleFlipCell({required this.animation, required this.child});
...
class _HunterdleNameCellContent extends StatelessWidget {
  final HxhCharacter character;
  final Color color;

  const _HunterdleNameCellContent({required this.character, required this.color});
...
class _HunterdleCellContent extends StatelessWidget {
  final String value;
  final Color color;

  const _HunterdleCellContent({required this.value, required this.color});
