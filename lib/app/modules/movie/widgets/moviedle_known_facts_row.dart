...
class MoviedleKnownFactsRow extends StatelessWidget {
  final List<MovieGuessResult> guesses;

  const MoviedleKnownFactsRow({super.key, required this.guesses});

  int _rank(MovieMatch match) {
    switch (match) {
  ...
  Widget build(BuildContext context) {
    final columnCount = movieColumnLabels().length;
    return Padding(
  ...
  Widget _summaryCell(int columnIndex) {
    ...
    final color = switch (best.match) {
      MovieMatch.correct => kMovieCorrectColor,
      MovieMatch.partial => kMoviePartialColor,
      MovieMatch.incorrect => kMovieIncorrectColor,
    };
    final display = best.match == MovieMatch.correct ? best.value : '?';
